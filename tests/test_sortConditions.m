...
%TEST_SORTCONDITIONS   Test that we sort initial/final conditions correctly.

%% First order scalar IVP
icFun = @(u) u;
correct = 1;
idx = treeVar.sortConditions(icFun, 1);
pass(1) = ( idx == correct );

%% Second order scalar IVP, increasing order
icFun = @(u) [u; diff(u)];
correct = [1 2];
idx = treeVar.sortConditions(icFun, 2);
...
correct = 1:6;
idx = treeVar.sortConditions(icFun, 6);
...
icFun = @(u,v) [u; v];
correct = [1 2];
idx = treeVar.sortConditions(icFun, [1 1]);
...
correct = [1 2 3 4];
idx = treeVar.sortConditions(icFun, [2 2]);
...
idx = treeVar.sortConditions(icFun, [4 4 4 4]);
pass(10) = all( idx == correct );

%% Unsupported format, multiplying unknown function
icFun = @(x,u) 5*u - 1;
try
    treeVar.sortConditions(icFun, 1);
catch ME
    % We're multiplying the unknown function -- this should give us an error.
...
try
    ...
    % We're multiplying the derivative of  the unknown function -- this should
    % give us an error.
...
icFun = @(x,u) u + diff(u);
try
    treeVar.sortConditions(icFun, 2);
catch ME
    % Two terms involving the unknown function -- this should give us an error.
...
try
    treeVar.sortConditions(icFun, [1 2]);
catch ME
    % Two terms involving the unknown functions -- this should give us an error.
...
try
    ...
    % Two conditions on the same variable -- this should give us an error.
...
try
    ...
    % Specifying conditions on a too hight order derivative -- this should give
    % us an error.
...
icFun = @(y) [y-1; diff(y,2)];
try
    treeVar.sortConditions(icFun, 3);
catch ME
    % Skipped specifying condition on y' -- this should give us an error.
...
try
    treeVar.sortConditions(icFun, [2 1]);
catch ME
    % Two conditions on v -- this should give us an error.
...
try
    treeVar.sortConditions(icFun, [2 2]);
catch ME
    % Specified a condition on v'' for a second order system -- this should give
    % us an error.
...
try
    treeVar.sortConditions(icFun, [3 3]);
catch ME
    % Skipped specifying condition on v -- this should give us an error.
...
try
    ...
    % Didn't specify condition on u'' and v'' for a third order system -- this
